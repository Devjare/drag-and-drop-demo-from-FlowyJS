var flowy = function(
	...
	onblockchange || (onblockchange = function() {});
		...
		if (!d) {
			...
			(flowy.endDrag = function(e) {
				...
				blocksDimensionDetails.push({
					...
					x: i.getBoundingClientRect().left + window.scrollX +
					parseInt(window.getComputedStyle(i).width) / 2 + html.scrollLeft,
					y: i.getBoundingClientRect().top + window.scrollY + 
					parseInt( window.getComputedStyle(i).height) / 2 + html.scrollTop,
					width: parseInt(window.getComputedStyle(i).width), 
					height: parseInt(window.getComputedStyle(i).height)
				});
				// this works as well
				// onblockchange('rearrange');
				...
				i.parentNode.removeChild(i);
					...
					for (var o = i.getBoundingClientRect().left +
						window.scrollX +parseInt(window.getComputedStyle(i).width) /2 + html.scrollLeft,
						n = i.getBoundingClientRect().top + window.scrollY + html.scrollTop,
						r = blocksDimensionDetails.map(e => e.id), d = 0;
						d < blocksDimensionDetails.length;
						d++) {
							if (
								o >= 
								blocksDimensionDetails.filter(e => e.id == r[d])[0].x -
								blocksDimensionDetails.filter(e => e.id == r[d])[0].width / 2 - v 
								&&
								o <= 
								blocksDimensionDetails.filter(e => e.id == r[d])[0].x + 
								blocksDimensionDetails.filter(e => e.id == r[d])[0].width / 2 + v 
								&&
								n >= 
								blocksDimensionDetails.filter(e => e.id == r[d])[0].y -
								blocksDimensionDetails.filter(e => e.id == r[d])[0].height / 2 
								&&
								n <=
								blocksDimensionDetails.filter(e => e.id == r[d])[0].y +
								blocksDimensionDetails.filter(e => e.id == r[d])[0].height) {

							(y = !1),
						c(i, !1, document.querySelector(".blockid[value='" + r[d] + "']").parentNode)
						? k(i, d, r) : ((y = !1),
						html.appendChild(document.querySelector(".indicator")),
						i.parentNode.removeChild(i));
						onblockchange('add');
						break;
					}
...
(flowy.moveBlock = function(e) {
	if ((e.targetTouches ? ((p = e.targetTouches[0].clientX), (w = e.targetTouches[0].clientY)) : ((p = e.clientX), (w = e.clientY)), L)) {
		...
		for (
			...
			// onblockchange('rearrange'); This works too
		}
...
// THIS onblockchange call is the one that is called whenever a block is 
// changed of parent
(blocksDimensionDetails = blocksDimensionDetails.concat(g)), onblockchange('reasign'),
(g = []);
} else
blocksDimensionDetails.push({
	childwidth: 0,
